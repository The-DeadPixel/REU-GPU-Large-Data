...
void matrixCpy(float *a, float *b, float *c, int tileLength, int m ) {
   ...
   for(int i = 0; i < tilesThatDivide; i++){
       ...
       //memcpy of tile C for host to device 
        ...
        for(int j = 0; j < tilesThatDivide; j++){
            ...
            //Fill tileA & tileB with elements from matrix A (COMPILER IS SAYING THESE ARE UNDEFINED WTF)
            fillA(Ta, a, tileLength, i, j);
            fillB(Tb, b, tileLength, i, j);
            ...
            //Do we need to free device A and B every iteration of inner loop? OR does thhis happen after inner loop terminates?
            cudaFree(d_a);
            cudaFree(d_b);
            free(Ta);
            free(Tb);
        ...
        //store c results back into main c matrix
 ...
 ****************************************************************/
void fillA(float *tileA, float *matrixA, int tileLength, int i, int j){
    //pointer declarations
    float **tilePtr = &tileA;
    float **matricPtr = &matrixA;
    
    for(int Ti = (tileLength*i); Ti < (tileLength*i)+tileLength; Ti++){
        for(int Tj = (tileLength*j); Tj < (tileLength*j) + tileLength; Tj++ ){
            *tilePtr[Ti * tileLength + Tj] = matricPtr[Ti][Tj]; 
 ...
 ****************************************************************/
void fillB(float *tileB, float *matrixB, int tileLength, int i, int j){
    //pointer declarations
    float **tilePtr = &tileB;
    float **matricPtr = &matrixB;
