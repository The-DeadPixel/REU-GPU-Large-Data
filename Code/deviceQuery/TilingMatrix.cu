...
void fillA(float *tileA, float *matrixA, int tileLength, int i, int j){
    ...
    for(int Ti = (tileLength*i); Ti < (tileLength*i)+tileLength; Ti++){
        for(int Tj = (tileLength*j); Tj < (tileLength*j) + tileLength; Tj++ ){
            tilePtr[Ti * tileLength + Tj] = matrixPtr[Ti * tileLength + Tj]; 
...
void matrixCpy(float *a, float *b, float *c, int tileLength, int m ) {
   ...
   for(int i = 0; i < m; i++){
       ...
       //memcpy of tile C for host to device (POSSIBLE AREA FOR TIMING)
        ...
        for(int j = 0; j < m; j++){
            printf("I MADE IT\n");
            /*Conditional that checks if tile indices cover area BELOW triangular line ie(only entries w/ all zeros), if so it skips these 
             indices and continues to next iteration*/
            if(j > i){
                continue;
            }
